...
import { DmLibraryView } from './DmLibraryView';

/**
 * Standalone "Monster Library" entry — meant to be opened in a second window /
 * monitor / tablet. It connects as another DM client (no new server state) and
 * renders the full-screen creature browser. The DM enters their secret here
 * (this is a separate window, so it doesn't share the main screen's login).
 */
export function DmLibraryRoute() {
  ...
  // Stay on the library through a reconnect blip (keep the last snapshot).
  if (
    snapshot?.role === 'dm' &&
    (status === 'connected' || status === 'reconnecting')
  ) {
    return <DmLibraryView />;
  }
