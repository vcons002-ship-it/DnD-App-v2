 ...
 * Roll20 has no public per-character export API.) The saved URL is per session
 * code — a different campaign usually means a different Roll20 game.
 */
export function Roll20Panel({ sessionCode }: { sessionCode: string }) {
  const urlKey = `roll20-url:${sessionCode}`;
  const [url, setUrl] = useState(() => localStorage.getItem(urlKey) ?? '');
  const [open, setOpen] = useState(false);

  const saveUrl = (u: string) => {
    setUrl(u);
    localStorage.setItem(urlKey, u);
  };
