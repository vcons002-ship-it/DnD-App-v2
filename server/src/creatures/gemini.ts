...
export async function lookupCreatureAI(
  ...
  const prompt =
    ...
    `"modelColor" is an optional overall color: red, blue, green, purple, black, white, gold, orange, pink, gray, or natural. Choose a color only when requested by the name/description or appearance tags, otherwise use an empty string. "modelType" is its physical creature family (goblin, skeleton, wolf, elephant, etc.), independent of its flavorful name or D&D category. "visualTags" are only requested visual themes/colors: fire, poison, ice, lightning, undead, red, blue, green, purple, black, white, gold. Bracket tags such as [fire] count. Do not infer tags from resistances or grant rules from visual tags. ` +
